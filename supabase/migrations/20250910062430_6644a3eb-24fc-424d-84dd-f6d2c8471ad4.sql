...
-- Drop the existing dangerous INSERT policy
...
FOR INSERT 
TO authenticated
WITH CHECK (false); -- Prevent all direct inserts

-- Allow the handle_new_user trigger to work by creating a service role policy
CREATE POLICY "System functions can insert roles" 
ON public.user_roles 
FOR INSERT 
TO service_role
WITH CHECK (true);

-- Keep the existing SELECT policy (users can view their own roles)
-- This policy already exists and is secure

-- Update the handle_new_user function to work with the new security model
...
BEGIN
  ...
  -- Default role is buyer - this bypasses RLS due to SECURITY DEFINER
  INSERT INTO public.user_roles (user_id, role)
  VALUES (NEW.id, 'buyer');
